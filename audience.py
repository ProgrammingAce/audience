...
- /screenshot   take a screenshot in 5 seconds
...
class Audience:
    ...
    def worker(self):
        while not self.stop.is_set():
            try:
                ...
                continue
            if kind == "commentary":
                self._do(question="Glance down from your perch at what the "
    ...
    def schedule_screenshot(self, delay=5):
        def go():
    ...
    def handle_submit(self, text):
        ...
        if text == "/screenshot":
            ...
            return
        if text == "/help":
            self.emit("commands: /screenshot, /quit  — or type a question",
                      style="hint")
            return
        if text.startswith("/"):
